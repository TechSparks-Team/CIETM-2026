...
import {
  ...
  ExternalLink, Mail, LogOut, CheckSquare, Square
} from 'lucide-react';
              ...
              <div className="flex flex-col md:flex-row gap-6">
                <div className="relative flex-1 group">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-6 py-4 bg-white rounded-2xl border border-slate-200 focus:outline-none focus:border-indigo-500 font-bold text-sm text-slate-700 transition-all shadow-sm"
                  />
                </div>
                <div className="flex gap-2 overflow-x-auto pb-2 md:pb-0 no-scrollbar">
                  {['All', 'Submitted', 'Under Review', 'Accepted', 'Rejected'].map(t => (
                    <button
                      key={t}
                      onClick={() => setFilter(t)}
                      className={`px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap border ${filter === t
          ...
          {activeTab === 'entry' && (
             <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-6xl mx-auto">
                <div className="bg-white rounded-[2.5rem] border border-slate-200 p-8 shadow-sm">
                   <h3 className="text-xl font-black text-slate-800 mb-6 flex items-center gap-3"><ScanLine className="text-indigo-600" /> Live QR Validator</h3>
                   <QRScanner onScan={handleVerifyQR} />
                </div>
                {scannedResult ? (
                   <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white rounded-[2.5rem] border border-slate-200 p-8 shadow-sm flex flex-col">
                      <div className="flex items-center gap-4 mb-6 pb-6 border-b border-slate-100">
                         <div className="w-16 h-16 rounded-2xl bg-indigo-600 text-white flex items-center justify-center text-2xl font-black uppercase">{scannedResult.personalDetails?.name?.charAt(0)}</div>
                         <div>
                      ...
                      <div className="space-y-4">
                         <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 grid grid-cols-2 gap-4">
                            <div><p className="text-[8px] font-bold text-slate-400 uppercase mb-1">ID Status</p><p className="text-xs font-bold text-blue-600 uppercase tracking-widest font-black">ACTIVE</p></div>
                            <div><p className="text-[8px] font-bold text-slate-400 uppercase mb-1">Payment Status</p><p className={`text-xs font-bold uppercase tracking-widest font-black ${scannedResult.paymentStatus === 'Completed' ? 'text-green-600' : 'text-red-600'}`}>{scannedResult.paymentStatus}</p></div>
                         </div>
                         {scannedResult.paymentStatus !== 'Completed' && (
                            <div className="mt-4 p-4 border-2 border-red-100 rounded-[2rem] bg-red-50/20">
                               <p className="text-[10px] font-black text-red-600 uppercase mb-3">On-site Payment Settlement</p>
                               <input type="number" placeholder="Enter Amount" value={manualPaymentAmount} onChange={(e) => setManualPaymentAmount(e.target.value)} className="w-full p-4 rounded-xl border-none outline-none font-black text-slate-700 mb-3" />
                               <button onClick={() => handleManualPaymentConfirm(scannedResult)} className="w-full py-3 bg-red-600 text-white rounded-xl font-black uppercase text-[10px] tracking-widest shadow-lg shadow-red-200">Collect & Verify</button>
                            </div>
                         )}
                         <button onClick={() => setScannedResult(null)} className="w-full py-3 text-slate-400 font-bold uppercase text-[10px] tracking-widest mt-4">Reset Scanner</button>
                      </div>
                   </motion.div>
                ) : (
                   <div className="flex flex-col items-center justify-center py-20 bg-white border border-slate-200 rounded-[2.5rem] shadow-sm opacity-50">
                      <QrCode size={48} className="text-slate-200 mb-4" />
                      <p className="text-sm font-black text-slate-400 uppercase tracking-widest">Waiting for Scan...</p>
                   </div>
                )}
             </div>
          )}
                            ...
                            {reviewers.map(rev => (
                               <tr key={rev._id} className="hover:bg-slate-50/50 transition-colors">
                                  <td className="px-8 py-5">
                                     <div className="flex items-center gap-3">
                                        <div className="w-8 h-8 rounded-lg bg-slate-100 text-slate-600 flex items-center justify-center font-bold text-xs uppercase">{rev.name.charAt(0)}</div>
                                        <div>
